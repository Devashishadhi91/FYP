...
module.exports.createCategory = async (req, res) => {
    try {
        ...
        if (req.user.role === 'staff') {
            return res.status(403).json({ message: "Staff are not allowed to create categories." });
        }
...
module.exports.RemoveCategory=async(req,res)=>{
    try {
        ...
        if (req.user.role === 'staff') {
            return res.status(403).json({ message: "Staff are not allowed to delete categories." });
        }
...
module.exports.updateCategory=async(req,res)=>{
    try {
        ...
        if (req.user.role === 'staff') {
            return res.status(403).json({ message: "Staff are not allowed to update categories." });
        }
